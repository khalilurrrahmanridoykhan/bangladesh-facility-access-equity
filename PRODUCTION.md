...
The Android app bundles the same public web assets. Native feedback requests are restricted to the Capacitor origin and sent to the production HTTPS API. Production deployment creates a release-signed APK and places it at `web/downloads/shasthopath-1.1.0.apk` before rebuilding the service image.
...
curl --fail --head https://shasthopath.krrkhan.com/downloads/shasthopath-1.1.0.apk
sha256sum web/downloads/shasthopath-1.1.0.apk
```
